...
class AlarmEngineTests(unittest.TestCase):
    ...
    def test_stale_alarm_transition(self) -> None:
        ...
        alert = self.engine.evaluate_staleness("borne-01", self.engine.stale_after_s + 1)
        self.assertEqual(alert[0]["code"], "AC_TELEMETRY_STALE")
        self.assertEqual(self.engine.evaluate_staleness("borne-01", self.engine.stale_after_s + 2), [])
