# Firmware 0.2.15-field : stabilisation du point d'accès local

Par rapport à 0.2.14-field, la borne lance son AP pendant 20 secondes avant
la première tentative Wi-Fi STA. Elle ne demande plus à la pile radio de
relancer elle-même les connexions ratées et réessaie le Wi-Fi de secours
toutes les cinq minutes lorsque l'AP est actif. Une recherche SN reste
effectuée par le firmware ; elle est plus espacée lorsqu'un appareil est
associé à l'AP. La file locale n'est pas effacée par la mise à jour USB.

Pour confirmer le problème initial : avec SN **éteint**, laisser la carte
alimentée par USB, surveiller pendant plus de cinq minutes le SSID
VE-SCOPE-borne-01 et vérifier que Windows peut s'y associer puis ouvrir
`http://192.168.4.1/api/connectivity`. Garder le moniteur série ouvert
et noter chaque ligne « Tentative Wi-Fi secours ». Si la liaison reste
impossible, comparer avec un téléphone ; rechercher ensuite les erreurs
de connexion et d'alimentation, sans reflasher à l'aveugle.
...
1. Confirmer sur le moniteur série `Boot firmware 0.2.15-field` et vérifier
